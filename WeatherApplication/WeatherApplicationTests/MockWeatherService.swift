...
class MockWeatherService: WeatherAPI {
    ...
    func getWeatherDataForcastFor(city cityName: String, completion: @escaping (ResponseHandler<WeatherMasterData, WeatherAppError>) -> Void) {
        if checkforSuccess {
            if let path = Bundle(for: MockWeatherService.self).path(forResource: "mockData", ofType: "json") {
                do {
    ...
    func getCurrentWeatherDataFor(city cityName: String, completion: @escaping (ResponseHandler<WeatherData, WeatherAppError>) -> Void) {
        if checkforSuccess {
            if let path = Bundle(for: MockWeatherService.self).path(forResource: "mockCurrectWeather", ofType: "json") {
                do {
                    let data = try Data(contentsOf: URL(fileURLWithPath: path), options: .mappedIfSafe)
