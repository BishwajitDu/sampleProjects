...
class WeatherDashboardViewModel: NSObject {
    ...
    func fetchCurrentWeatherFromAPI() {
        ...
        weatherService?.getCurrentWeatherDataFor(city: self.cityName ?? "", completion: { [weak self] response in
    ...
    func fetchMasterWeatherDataFromAPI() {
        weatherDashboardSemaphore.wait()
        weatherService?.getWeatherDataForcastFor(city: self.cityName ?? "", completion: { [weak self] response in
            guard let strongSelf = self else {return}
            switch response {
            case .Success(let data):
